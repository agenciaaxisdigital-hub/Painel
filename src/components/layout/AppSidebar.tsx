        ...
        {modules.map((m) => {
          ...
          return (
            <NavLink key={m.path} to={m.path} className="flex flex-1 flex-col items-center gap-0.5 py-2">
              ...
              <span className={cn("text-[9px]", active ? "text-primary font-medium" : "text-muted-foreground")}>{m.label.split(" ")[0]}</span>
            </NavLink>
          );
        })}
      </div>
          ...
          {!collapsed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-muted-foreground">Painel da Campanha</span>
              </div>
            </motion.div>
          )}
          {!collapsed && <AlertBell />}
