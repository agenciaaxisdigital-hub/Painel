  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [v, f, wh, ig, fb] = await Promise.all([
        supabase.from("acessos_site").select("id").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
        supabase.from("mensagens_contato").select("id").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", since).eq("tipo_clique", "whatsapp").or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", since).eq("tipo_clique", "instagram").or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", since).eq("tipo_clique", "facebook").or(BRASIL_FILTER).limit(5000),
      ]);
      const visitantes = (v.data || []).length;
      const formularios = (f.data || []).length;
      const whatsapp = (wh.data || []).length;
      const instagram = (ig.data || []).length;
      const facebook = (fb.data || []).length;
      return {
        visitantes,
        formularios,
        whatsapp,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [cV, pV, cF, pF, cW, pW, cI, pI] = await Promise.all([
        supabase.from("acessos_site").select("id").gte("criado_em", currentSince).or(BRASIL_FILTER).limit(5000),
        supabase.from("acessos_site").select("id").gte("criado_em", previousSince).lt("criado_em", currentSince).or(BRASIL_FILTER).limit(5000),
        supabase.from("mensagens_contato").select("id").gte("criado_em", currentSince).or(BRASIL_FILTER).limit(5000),
        supabase.from("mensagens_contato").select("id").gte("criado_em", previousSince).lt("criado_em", currentSince).or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", currentSince).eq("tipo_clique", "whatsapp").or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", previousSince).lt("criado_em", currentSince).eq("tipo_clique", "whatsapp").or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", currentSince).eq("tipo_clique", "instagram").or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("id, tipo_clique").gte("criado_em", previousSince).lt("criado_em", currentSince).eq("tipo_clique", "instagram").or(BRASIL_FILTER).limit(5000),
      ]);
      const calc = (curr: number, prev: number) => (prev > 0 ? ((curr - prev) / prev) * 100 : curr > 0 ? 100 : 0);
      return {
        visitantes: calc((cV.data || []).length, (pV.data || []).length),
        formularios: calc((cF.data || []).length, (pF.data || []).length),
        whatsapp: calc((cW.data || []).length, (pW.data || []).length),
        instagram: calc((cI.data || []).length, (pI.data || []).length),
      };
    },
    staleTime: 60_000,
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [vis, forms, clicks] = await Promise.all([
        ...
        supabase.from("mensagens_contato").select("criado_em").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("criado_em, tipo_clique").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
      ]);
...
export function useDeviceBreakdown(days: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const since = getSince(days);
      const { data } = await supabase.from("acessos_site").select("dispositivo").gte("criado_em", since).or(BRASIL_FILTER).limit(5000);
...
export function useTrafficOrigin(days: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const since = getSince(days);
      const { data } = await supabase.from("acessos_site").select("referrer, utm_source").gte("criado_em", since).or(BRASIL_FILTER).limit(5000);
...
export function useClickPlatforms(days: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const since = getSince(days);
      const { data } = await supabase.from("cliques_whatsapp").select("tipo_clique").gte("criado_em", since).or(BRASIL_FILTER).limit(5000);
...
export function useTopCities(days: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const since = getSince(days);
      const { data } = await supabase.from("acessos_site").select("cidade").gte("criado_em", since).or(BRASIL_FILTER).limit(5000);
      const counts: Record<string, number> = {};
      (data || []).forEach((r) => {
        const cidade = r.cidade?.trim() || "Não identificada";
...
export function useTopPages(days: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const since = getSince(days);
      const { data } = await supabase.from("acessos_site").select("pagina").gte("criado_em", since).or(BRASIL_FILTER).limit(5000);
      const counts: Record<string, number> = {};
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const [v, f, c] = await Promise.all([
        supabase.from("acessos_site").select("criado_em").or(BRASIL_FILTER).order("criado_em", { ascending: false }).limit(200),
        supabase.from("mensagens_contato").select("criado_em").or(BRASIL_FILTER).order("criado_em", { ascending: false }).limit(200),
        supabase.from("cliques_whatsapp").select("criado_em").or(BRASIL_FILTER).order("criado_em", { ascending: false }).limit(200),
      ]);
      const allData = [...(v.data || []), ...(f.data || []), ...(c.data || [])];
      const lastRecord = allData.map(r => r.criado_em).sort().reverse()[0] || null;
      ...
      return {
        ...
        counts: { visitantes: (v.data || []).length, formularios: (f.data || []).length, cliques: (c.data || []).length },
      };
    },
    staleTime: 30_000,
...
export function useRegionCounts(days: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const since = getSince(days);
      const [vis, forms, clicks] = await Promise.all([
        supabase.from("acessos_site").select("regiao_planejamento").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
        supabase.from("mensagens_contato").select("regiao_planejamento").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
        supabase.from("cliques_whatsapp").select("regiao_planejamento").gte("criado_em", since).or(BRASIL_FILTER).limit(5000),
      ]);
